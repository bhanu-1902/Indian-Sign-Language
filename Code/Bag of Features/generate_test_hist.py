...
test_folder='D:/Indian Sign Language/Code/Train-Test/Test'

# Loading Test images
test_images=load_images_by_category(test_folder)

#Extract SURF features from the image
surf_test=surf_features(test_images)
all_train_descriptors=surf_test[0]

kmeans=minibatchkmeans(n_classes*clustering_factor,all_train_descriptors)

#print(len(surf_test['a'][0]))

# Create histograms from extracted surf features
bows_test=create_histogram(surf_test[1],kmeans)

import csv
loc='D:/Indian Sign Language/Code/Code/Classification/csv files/test.csv'
